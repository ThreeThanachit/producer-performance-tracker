...
|---|---|---|
| `SessionBuilder_AppsScript.js` | System A | Build Sessions → Master Log (APPEND-ONLY) → Today/Overdue/Weekly/Monthly + race-safe sync + Producer Assignments |
...
Raw Schedule (external) → buildSessionsSheet() → Sessions tab
   → buildMasterLog() [APPEND-ONLY dispatcher]
     ...
     └─ ML มี data → _appendNewSessionsToMasterLog()
                      ↑ auto-fill Senior PIC จาก Producer Assignments
...
- `_appendNewSessionsToMasterLog` lookup → auto-fill column E ตอน append
- Producer ไม่มีใน mapping → Senior ว่าง (กรอกเองได้)
- Menu: "👥 Setup Producer → Senior assignments" — สร้าง tab + เพิ่ม producer ที่ยังไม่ assign
...
| Senior PIC manual fill every time | Producer Assignments auto-fill table |
...
| ✅ Phase 10 — Producer→Senior auto-assignment | Done |
